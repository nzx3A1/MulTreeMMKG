...
_SYMMETRIC_RELATIONS = {"OVERLAPS", "ADJACENT_TO"}
...
def normalize_map_spatial_result(
    ...
    """清洗模型 JSON、稳定局部 ID、规范类型/数值及四类空间关系。"""

    result = _sanitize(deepcopy(dict(payload)))
    result["schema_version"] = "map_spatial.v1"
    ...
    for raw in _items(result.get("spatial_relations")):
        ...
        relation_key = re.sub(r"[^A-Z]+", "_", _text(item.get("type")).upper()).strip("_")
        relation_type = _SPATIAL_RELATION_ALIASES.get(relation_key)
        if not source_id or not target_id or source_id == target_id or not relation_type:
            continue
        if relation_type == "LOCATED_IN":
            membership = (source_id, target_id)
        elif relation_type == "CONTAINS":
            membership = (target_id, source_id)
        else:
            membership = None
        ...
        dedupe_endpoints = tuple(sorted((source_id, target_id))) if relation_type in _SYMMETRIC_RELATIONS else (source_id, target_id)
        key = (dedupe_endpoints[0], relation_type, dedupe_endpoints[1])
        if key in seen:
            continue
        seen.add(key)
        item.update(
        ...
        graph.metadata.extra.update(
            {
                ...
                "algorithm": "VLM证据抽取 + 类型/拓扑规范化 + 确定性Graph装配",
    ...
    def _relation(
        ...
        *,
        ...
        self.relations[relation_id] = Relation(
            ...
            attributes={
                "dimension": "spatial" if is_spatial else "semantic",
