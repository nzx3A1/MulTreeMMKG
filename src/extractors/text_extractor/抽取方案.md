...
1. `source_id` 和 `target_id` 均对应当前 Chunk Graph 中的实体；
2. `source_name`、`source_type` 分别等于源实体的 `name`、`type`；
3. `target_name`、`target_type` 分别等于目标实体的 `name`、`type`；
4. 解析器只校验这些字段的格式，并按实体 ID 引用同步规范值，不校验关系是否属于白名单；
5. 不能把“可能、推测、尚有争议”改写成确定事实，语气应保存到 `attributes`，如
   `{"certainty": "推测"}`。
...
| 校验项 | 处理方式 |
|---|---|
| 实体或事件类型不在局部概念白名单 | 不校验；非空字符串且字段格式正确即可 |
| 关系类型、方向或端点类型不在局部关系白名单 | 不校验；类型与端点引用格式正确即可 |
| `provenance` 不是当前正文的子串 | 不校验内容，只检查字段为字符串 |
| 关系端点不存在 | 拒绝关系 |
...
4. **格式校验**：白名单外类型可以通过，但缺少必填字段或字段类型错误会被记录；
5. **端点同步**：关系的源/目标名称与类型始终和对应实体 ID 引用保持一致；
